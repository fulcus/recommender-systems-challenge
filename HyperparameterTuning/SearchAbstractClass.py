...
class SearchAbstractClass(object):
    ...
    def _set_search_attributes(self, recommender_input_args,
        ...
        if save_model not in self._SAVE_MODEL_VALUES:
            raise ValueError("{}: argument save_model must be in '{}', provided was '{}'.".format(self.ALGORITHM_NAME,
                                                                                                  self._SAVE_MODEL_VALUES,
                                                                                                  save_model))

        if evaluate_on_test not in self._EVALUATE_ON_TEST_VALUES:
            raise ValueError(
                "{}: argument evaluate_on_test must be in '{}', provided was '{}'.".format(self.ALGORITHM_NAME,
                                                                                           self._EVALUATE_ON_TEST_VALUES,
                                                                                           evaluate_on_test))
    ...
    def _init_metadata_dict(self, n_cases, hyperparameter_names):
        ...
        self.metadata_dict = {"algorithm_name_search": self.ALGORITHM_NAME,
                              ...
                              "hyperparameters_df": pd.DataFrame(columns=hyperparameter_names, index=np.arange(n_cases),
                                                                 dtype=object),
                              ...
                              "time_df": pd.DataFrame(columns=["train", "validation", "test"],
    ...
    def _remove_intermediate_cases(self, cases_to_remove_list):
        ...
        self.metadata_dict["time_on_validation_avg"] = self.metadata_dict["time_df"]["validation"].mean(axis=0,
                                                                                                        skipna=True)
        self.metadata_dict["time_on_validation_total"] = self.metadata_dict["time_df"]["validation"].sum(axis=0,
                                                                                                         skipna=True)
    ...
    def _evaluate_on_validation(self, current_fit_hyperparameters, was_already_evaluated_flag,
        ...
        if not was_already_evaluated_flag:
            ...
            self.metadata_dict["hyperparameters_df"].loc[was_already_evaluated_index].copy()
    ...
    def _evaluate_on_test_with_data_last(self):
        ...
        if self.resume_from_saved and self.metadata_dict["result_on_last"] is not None:
            self._print("{}: Resuming '{}'... Result on last already available.".format(self.ALGORITHM_NAME,
                                                                                        self.output_file_name_root))
            return

        self._print(
            "{}: Evaluation with constructor data for final test. Using best config: {}".format(self.ALGORITHM_NAME,
                                                                                                self.metadata_dict[
        ...
        if self.save_model in ["all", "best", "last"]:
            ...
            recommender_instance.save_model(self.output_folder_path,
    ...
    def _objective_function(self, current_fit_hyperparameters_dict):
        ...
        try:
            ...
            was_already_evaluated_flag, was_already_evaluated_index = self._was_already_evaluated_check(
                current_fit_hyperparameters_dict)
            result_df, recommender_instance = self._evaluate_on_validation(current_fit_hyperparameters_dict,
            ...
            current_fit_hyperparameters_dict = self.metadata_dict["hyperparameters_df"].loc[
                self.model_counter].to_dict()
