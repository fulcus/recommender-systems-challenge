...
from Recommenders.Hybrids.scores.ScoresHybridKNNCFKNNCBF import ScoresHybridKNNCFKNNCBF
from Recommenders.Hybrids.scores.ScoresHybridP3alphaKNNCBF import ScoresHybridP3alphaKNNCBF
from Recommenders.Hybrids.scores.ScoresHybridP3alphaPureSVD import ScoresHybridP3alphaPureSVD
from Recommenders.Hybrids.scores.ScoresHybridRP3betaKNNCBF import ScoresHybridRP3betaKNNCBF
from Recommenders.Hybrids.scores.ScoresHybridSpecializedAdaptive import ScoresHybridSpecializedAdaptive
from Recommenders.Hybrids.scores.ScoresHybridUserKNNCFKNNCBF import ScoresHybridUserKNNCFKNNCBF
...
def runHyperparameterSearch_Hybrid(recommender_class, URM_train, W_train, ICM_object, ICM_name,
    ...
    try:
        ...
        if recommender_class is HybridRatings_EASE_R_hybrid_SLIM_Rp3:
            ...
            hyperparameters_range_dictionary["beta"] = Real(low=0.05, high=0.15, prior='uniform')
            hyperparameters_range_dictionary["alpha1"] = Real(low=0.8, high=0.99, prior='uniform')
            hyperparameters_range_dictionary["beta1"] = Real(low=0.05, high=0.2, prior='uniform')
        ...
        if recommender_class is Hybrid_SLIM_EASE_R_IALS:
            hyperparameters_range_dictionary = {}
            hyperparameters_range_dictionary["alpha"] = Real(low=0.0, high=1.0, prior='uniform')
            hyperparameters_range_dictionary["beta"] = Real(low=0.0, high=1.0, prior='uniform')
            hyperparameters_range_dictionary["gamma"] = Real(low=0.0, high=1.0, prior='uniform')
    ...
    try:
        ...
        if recommender_class is IALSRecommender:
            hyperparameters_range_dictionary = {
                ...
                "reg": Real(low=1e-5, high=0.2, prior='log-uniform'),
            }
        ...
        if recommender_class is IALSRecommender_implicit:
            hyperparameters_range_dictionary = {
                "n_factors": Integer(1, 200),
                "regularization": Real(low=1e-5, high=0.15, prior='log-uniform'),
            }
            ...
            recommender_input_args = SearchInputRecommenderArgs(
                ...
                CONSTRUCTOR_KEYWORD_ARGS={},
                FIT_POSITIONAL_ARGS=[],
                FIT_KEYWORD_ARGS={}  # if necessary add here earlystopping
            )
        ...
        if recommender_class is SLIMElasticNetRecommender:  # or recommender_class is MultiThreadSLIM_SLIMElasticNetRecommender:
            hyperparameters_range_dictionary = {
                ...
                "alpha": Real(low=0.01, high=0.5, prior='uniform'),  # (low=1e-3, high=1.0, prior='uniform'),
