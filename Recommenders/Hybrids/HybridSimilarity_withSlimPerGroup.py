...
class HybridSimilarity_withSlimPerGroup(BaseItemSimilarityMatrixRecommender):
    ...
    RECOMMENDER_NAME = "HybridSimilarity_withSlimPerGroup"

    def __init__(self, URM_train):
        super(HybridSimilarity_withSlimPerGroup, self).__init__(URM_train)

        slim_g0 = SLIMElasticNetRecommender(URM_train)
        ...
        self.recommender_1 = slim_g0
        self.recommender_2 = slim_g1
        self.recommender_3 = rp3
    ...
    def fit(self, topK=1190, alpha=0.96):
        print('{} hyperparams: topK: {}; alpha: {}'.format(self.RECOMMENDER_NAME, str(topK), alpha))

        self.topK = topK
        self.alpha = alpha

        W_sparse_0 = self.similarity_1 * self.alpha + self.similarity_3 * (1 - self.alpha)
        W_sparse_1 = self.similarity_2 * self.alpha + self.similarity_3 * (1 - self.alpha)
